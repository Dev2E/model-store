@echo off
REM ============================================
REM GERAR CERTIFICADOS SSL PARA DESENVOLVIMENTO (Windows)
REM ============================================
REM Este script cria certificados self-signed para HTTPS em desenvolvimento
REM Requer OpenSSL instalado

echo 🔐 Gerando certificados SSL para desenvolvimento...

REM Criar pasta .ssl
if not exist .ssl mkdir .ssl

REM Gerar chave privada e certificado (válido por 365 dias)
openssl req -x509 -newkey rsa:2048 -keyout .ssl/server.key -out .ssl/server.crt ^
  -days 365 -nodes ^
  -subj "/C=BR/ST=SP/L=São Paulo/O=Vellenia/CN=localhost"

if %ERRORLEVEL% EQU 0 (
  echo ✅ Certificados gerados com sucesso!
  echo 📁 Localização: .ssl\server.key e .ssl\server.crt
  echo.
  echo Para usar HTTPS em desenvolvimento:
  echo    set VITE_HTTPS=true ^&^& npm run dev
  echo.
  echo ⚠️  Aviso:
  echo    - Certificado é self-signed (não reconhecido por navegadores)
  echo    - Ignorar aviso de segurança no navegador ^(é seguro para DEV^)
  echo    - Produção: Vercel gerencia SSL automaticamente
) else (
  echo ❌ Erro ao gerar certificados!
  echo Make sure OpenSSL is installed and in PATH
  echo Download: https://slproweb.com/products/Win32OpenSSL.html
)

pause
